...
# one regression for expenditures and revenue, and one for lending

regression_1_er_disp = lm(gini_disp ~ expenditures + revenue + gdp_growth_rate + 
                 fbinpercGDP + unemp_rate + EURO
               + factor(year) + factor(country), data = data_v6)
summary(regression_1_er_disp)

regression_2_l_disp = lm(gini_disp ~ lending + gdp_growth_rate + 
                    fbinpercGDP + unemp_rate + EURO
                  + factor(year) + factor(country), data = data_v6)
summary(regression_2_l_disp)

regression_3_er_mkt = lm(gini_mkt ~ expenditures + revenue + gdp_growth_rate + 
                            fbinpercGDP + unemp_rate + EURO
                          + factor(year) + factor(country), data = data_v6)
summary(regression_3_er_mkt)

regression_4_l_mkt = lm(gini_mkt ~ lending + gdp_growth_rate + 
                           fbinpercGDP + unemp_rate + EURO
                         + factor(year) + factor(country), data = data_v6)
summary(regression_4_l_mkt)



# refining the regression
# weiß nicht, ob das überhaupt nötig ist und wie gut usner modell sein muss.
# die ergebnisse der ersten beiden regressionen sind nämlich ziemlcih gut

regression_5 <- plm(formula = log(gini_mkt) ~ expenditures + revenue + gdp_growth_rate + 
...
ggplot(
  data.frame(
    gefitteteWerte = fitted(regression_1_er_disp),
    Residuen = residuals(regression_1_er_disp)),
  aes(x=gefitteteWerte, y=Residuen)) + 
  ggtitle("Tukey-Anscombe-Plot Regression 1") +
...
ggplot(
  data.frame(
    gefitteteWerte = fitted(regression_2_l_disp),
    Residuen = residuals(regression_2_l_disp)),
  aes(x=gefitteteWerte, y=Residuen)) + 
  ggtitle("Tukey-Anscombe-Plot Regression 2") +
  geom_hline(yintercept = 0) +
  geom_point()
...
ggplot(
  data.frame(
    gefitteteWerte = fitted(regression_3_er_mkt),
    Residuen = residuals(regression_3_er_mkt)),
  aes(x=gefitteteWerte, y=Residuen)) + 
  ggtitle("Tukey-Anscombe-Plot Regression 3") +
  geom_hline(yintercept = 0) +
  geom_point()
...
ggplot(
  data.frame(
    gefitteteWerte = fitted(regression_4_l_mkt),
    Residuen = residuals(regression_4_l_mkt)),
  aes(x=gefitteteWerte, y=Residuen)) + 
  ggtitle("Tukey-Anscombe-Plot Regression 4") +
  geom_hline(yintercept = 0) +
  geom_point()
...
ggplot(
  data.frame(
    gefitteteWerte = fitted(regression_5),
    Residuen = residuals(regression_5)),
  aes(x=gefitteteWerte, y=Residuen)) + 
  ggtitle("Tukey-Anscombe-Plot Regression 5") +
  geom_hline(yintercept = 0) +
  geom_point()

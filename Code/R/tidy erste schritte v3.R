...
wdi_growth_tidy <- tibble(wdi_growth_tidy)

wdi_growth_tidy <- wdi_growth_tidy %>% 
  mutate(year = as.double(year))
...
wdi_growth_tidy <- mutate_if(wdi_growth_tidy,is.numeric, round, digits = 2)
 ...
 #noch dezimalstellen begrenzen 
  
# controlling for foreign balances
